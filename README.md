# myhomevr
A little Unity VR scene representing my real house made from scratch. It can be run either with Google Cardboard (GoogleVR already included) or a SteamVR-compatible headset as HTC Vive, Oculus Rift or Microsoft Mixed Reality Headsets (SteamVR must be installed in the system). The scene "LivingRoom 3 DoF" can be un with any headset, and the scene "LivingRoom 6 DoF" can be run just with an OpenVR-enabled headset as it requires motion controllers and 6 DoF tracking.
